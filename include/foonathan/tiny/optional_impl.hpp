...
namespace foonathan
{
    namespace tiny
    {
        ...
        struct tombstone_traits<optional_impl<T>>
        {
            using overlap_spare_bits =
                typename std::conditional<optional_impl<T>::is_compressed::value,
                                          ...
                                          std::false_type,
                                          tombstone_overlaps_spare_bits<bool>>::type;
            ...
            {
                create_tombstone(typename optional_impl<T>::is_compressed{}, memory,
                                 tombstone_index);
            }
        ...
        /// \notes As `optional_impl` is not copyable, it cannot actually be used directly as a type with spare bits.
        /// But the proper optional type can delegate to these traits.
        template <typename T>
