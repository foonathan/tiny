...
namespace foonathan
{
    namespace tiny
    {
        namespace detail
        {
            ...
            template <typename... Ts>
            using pointer_variant_tag = pointer_variant_tag_impl<
                (1u << spare_bits<detail::pointer_variant_storage<Ts...>>()) >= sizeof...(Ts)>;
        ...
        /// The storage implementation of a variant of pointer to one of the types that uses spare bits whenever possible.
        ///
        ...
        public:
            ...
            ///
            /// `::is_valid` is [std::true_type]() if a pointer to the element type can be stored, [std::false_type]() otherwise.
