...
namespace foonathan
{
    namespace tiny
    {
        ...
        struct tombstone_traits_default
        {
            ...
            /// \returns The index of the given tombstone, if `memory` points to an address where the bit pattern corresponds to a tombstone.
            /// A value `>= tombstone_count` otherwise.
            /// \requires `memory` points to memory where either a valid `T` object has been constructed or a tombstone.
        ...
        /// \notes If this implementation is used tombstones cannot be distinguished from objects where the spare bits are used.
        template <typename T>
